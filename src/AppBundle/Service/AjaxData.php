    ...
    public function getScoreOrderedPersons(Request $request, $router)
    {
        ...
        if (isset($users['result']) && sizeof($users['result']) > 0) {
            foreach ($users['result'] as $k => $u) {
                ...
                $matches[] = Countries::getName($u['from_country']);//(isset($u['from_country'])&&($u['from_country']==$users['user']->getFrom()))?'<span class="matches">'.Countries::getName($u['from_country']).'</span>':Countries::getName($u['from_country']);
                $matches[] = (isset($u['municipality_id']) && ($u['municipality_id'] == $municipality->getId())) ? '<span class="matches">'.$municipality->getName().'</span>' : $municipality->getName();
                $matches[] = (isset($u['has_children']) && ($u['has_children'] == $users['user']->hasChildren()) && ($u['has_children'] == 1)) ? '<span class="matches">kids</span>' : 'no kids';
                $interests = [];
                ...
                $row .= '</div>';
                $row .= '<h4>'.trim(sprintf('%s %s', $u['first_name'], $u['last_name'])).' ('.implode(', ', $matches).')</h4>';
